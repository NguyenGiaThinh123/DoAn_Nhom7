...
namespace QuanLyCaPheApp.ViewModels
{
    public class DatBanViewModel : BaseViewModel
    {
        ...
        public void Load()
        {
            ...
            var tatCaBan = _banRepo.GetAll();

            // Lọc bàn: đưa về chữ thường và cắt khoảng trắng để so sánh
            var banHopLe = tatCaBan.Where(b => {
                string status = b.TrangThai?.ToLower().Trim() ?? "";
                return status == "trống" || status == "trong" ||
                       status == "đã đặt" || status == "đặt trước" || status == "da dat";
            }).ToList();

            DanhSachBan = new ObservableCollection<Ban>(banHopLe);
